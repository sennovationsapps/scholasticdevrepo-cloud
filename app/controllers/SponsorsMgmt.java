...
/*	public static Result get(Event event, Long pageId) {
	...
	public static Result get(Event event, Long pageId) {
		System.out.println("within get for view sponsor");
		if (event.isIdOnly()) {
			event = Event.findByIdWithMin(event.id);
		}
		final User localUser = ControllerUtil.getLocalUser(session());
		...
		/*=========new addition============ start========27.08.2015================
		List<SponsorItem> previousItemList = sponsors.getSponsorsItemWithDonatedSponsorUnmodified();
		System.out.println("sponsors.previousItem:: "+previousItemList);
